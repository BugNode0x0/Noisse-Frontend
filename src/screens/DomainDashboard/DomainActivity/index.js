...
import styles from "./DomainActivity.module.sass";
...
const DomainActivity = () => {
  ...
  useEffect(() => {
    const fetchCountData = async () => {
      try {
        const discoveredData = await getDiscoveredDomainsCount('week'); // Replace 'week' with your desired interval
        const activeData = await getActiveDomainsCount('week');
        const webData = await getWebDomainsCount('week');
        

        // Update state with the fetched data
        setDiscoveredCount(discoveredData);
        setActiveCount(activeData);
        setWebCount(webData);
      } catch (error) {
    ...
    >
      <div className={styles.table}>
        <div className={styles.row}>
          ...
          <div className={styles.col}>Web</div>
        </div>
        {items.map((x, index) => (
